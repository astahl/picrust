...
impl Debug for VideoInputParameters {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        if let Some(digital) = self.try_as_digital() {
            write!(f,"{:?}", digital)
        } else if let Some(analog) = self.try_as_analog() {
            write!(f,"{:?}", analog)
        } else {
            Ok(())
        }
...
bit_field!(pub EstablishedTimingModes0 (u8) {
...
bit_field!(pub EstablishedTimingModes1 (u8) {
...
bit_field!(pub EstablishedTimingModes2 (u8) {
7 => supports_1152x870v75
});

bit_field!(pub StandardTiming (u16) {
