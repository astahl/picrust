...
pub fn get_arm_memory() -> Option<Memory> {
    ...
    mb.push_request_raw(mailbox::Tag::HwGetArmMemory as u32, 8).ok()?;
    let mut responses = mb.submit_messages(8).ok()?;
    let (base_address, size): (u32, u32) = *responses.nth(0)?.ok()?.try_value_as()?;
    Some(Memory {
...
pub fn get_board_info() -> Option<BoardInfo> {
    let mut mb = mailbox::Mailbox::<256>::new();
    mb.push_request_raw(mailbox::Tag::HwGetBoardModel as u32, 4).ok()?;
    mb.push_request_raw(mailbox::Tag::HwGetBoardRevision as u32, 4).ok()?;
    mb.push_request_raw(mailbox::Tag::HwGetBoardSerial as u32, 8).ok()?;
    let mut responses = mb.submit_messages(8).ok()?;
